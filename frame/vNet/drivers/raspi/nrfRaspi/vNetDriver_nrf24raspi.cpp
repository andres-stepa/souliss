...
RF24 radio(NRF24_RADIOEN, NRF24_SPICS, BCM2835_SPI_SPEED_1MHZ);
...
{
    if(len > VNET_MAX_PAYLOAD) return VNET_DATA_FAIL;

    // If is a vNet broadcast, move on the broadcast pipe
    if((addr == VNET_ADDR_wBRDC) || (addr == VNET_ADDR_nBRDC)) addr = VNET_ADDR_BRDC;

    
    oFrame_Define(frame);				// Set the frame
    uint8_t flen = oFrame_GetLenght();			// Get the total length	
